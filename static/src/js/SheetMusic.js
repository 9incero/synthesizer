...
let melody_clip = new MusicClip(
  MusicClipType.Melody,
  Melody_clip_array.length,
  duration
);
let beat_clip = new MusicClip(
  MusicClipType.Beat,
  Beat_clip_array.length,
  duration
);
let template_clip = new TemplateClip(
  Template_clip_array.length,
  duration_track
);
...
createVerticalGrid(
  MusicClipType.Melody,
  time_to_px_Scale(saveStepTime, duration, clip_box_width - clip_start_px, 0)
);
createVerticalGrid(
  MusicClipType.Beat,
  time_to_px_Scale(saveStepTime, duration, clip_box_width - clip_start_px, 0)
...
function totalTypeSceneChanger() {
  if (
    window.confirm(
      "마지막 페이지로 가면 감상 및 저장만 가능하고 더 이상 편집이 불가능합니다. 다음 페이지로 넘기시겠습니까?"
    )
  ) {
    ...
    document.getElementById("sheetMusicSaveButton").style.marginBottom =
      "-400px;";
...
document
  .getElementById("recordStartButton")
  .addEventListener("click", function () {
    let timeElapsed = 4;
    const intervalId = setInterval(function () {
      ...
      document.getElementById(
        "videoWaitTime"
      ).innerHTML = `Recording Starts In ${timeElapsed} Seconds.`;
      if (timeElapsed <= 0) {
...
document
  .getElementById("recordStopButton")
  .addEventListener("click", function () {
    // 녹화 종료!
...
function createVideoClipObject(videoCLipId) {
  ...
  videoIdClip.addEventListener("click", function () {
    ...
    if (current_clip_type == MusicClipType.Lyrics) {
      document.getElementById("lyricsVideo").innerHTML =
        videoIdClip.textContent;
    }
  });
  ...
  videoIdClip.addEventListener("click", function () {
    ...
    if (clickedVideoHistory.length == 1) {
      // 하나만 들어왔을 시
    ...
    // 두개 이상 들어왔을 시
      ...
      clickedVideoHistory.length > 1 &&
      clickedVideoHistory[clickedVideoHistory.length - 1] !=
        clickedVideoHistory[clickedVideoHistory.length - 2]
    ) {
      nowVideoClicked = document.getElementById(
        clickedVideoHistory[clickedVideoHistory.length - 1]
      );
      previousVideoClicked = document.getElementById(
        clickedVideoHistory[clickedVideoHistory.length - 2]
      );
...
document
  .getElementById("lyricsSettingButton")
  .addEventListener("click", function () {
    if (current_clip_type == MusicClipType.Lyrics) {
      ...
      if (noteClickIndex != -1) {
        ...
        alert(
          "자막(" +
            melody_clip.getLyricsText(noteClickIndex) +
            ")과 영상 (" +
            lyricsText +
            ")이 연결되었습니다."
        );
        melody_clip.setLyricsVideo(noteClickIndex, lyricsId);
        videoClipObjectNameChange(
          lyricsId,
          melody_clip.getLyricsText(noteClickIndex)
        );
      }
    ...
    if (lyricsIndex != -1 && videoId != -1) {
      ...
      if (lyricsIndex != -1 && previousLyricsIndex != lyricsIndex) {
        ...
        setViodeTime(
          ...
          videoDuration / (lastTime - startTime)
        );
      }
    }
    // else{
  ...
  for (let trackTimer = 0; trackTimer < duration_track; trackTimer += 1 / fps) {
    ...
    if (cur_track_set[1].length > 0 && miditype == MusicClipType.Melody) {
      MidiMelodyMaker(
        track,
        trackTimer,
        trackTimer - cur_track_set[1][0],
        Melody_clip_array[cur_track_set[1][1]]
      );
    }
    if (cur_track_set[2].length > 0 && miditype == MusicClipType.Beat) {
      MidiBeatMaker(
        track,
        trackTimer,
        trackTimer - cur_track_set[2][0],
        Beat_clip_array[cur_track_set[2][1]]
      );
    }
  }
}
...
function MidiNoteAdditor(track, currentNote, previousNote, currentTime) {
  //피아노 음 재생 함수
  ...
  if (exclusiveArr1.length > 0) {
    ...
    for (let note of exclusiveArr1) {
      track.noteOn(
        0,
        note,
        parseInt((currentTime - MidiEventTime) * midiBPM * 2)
      );
      console.log(
        "note ON",
        note,
...
function InitializeAllSetting() {
  ...
  melody_clip = new MusicClip(
    MusicClipType.Melody,
    Melody_clip_array.length,
    duration
  );
  beat_clip = new MusicClip(
    MusicClipType.Beat,
    Beat_clip_array.length,
    duration
  );
...
function updateTime() {
  ...
  if (bpmPlayNumber > 4) {
    ...
    $("#slider").slider(
      "value",
      time_to_px(currentTime, currentClipDuration())
    );
    document.getElementById("clipTimeShower").innerText =
      getTimeString(currentTime);
    timeLine2.style.left =
      time_to_px(currentTime, currentClipDuration()) - clip_start_px + "px";
    timeLine1.style.left =
      time_to_px(currentTime, currentClipDuration()) - clip_start_px + "px";
...
function px_to_time_Scale(
  px,
  duration,
  width = clip_box_width,
  start = clip_start_px
) {
  //Px Scale을 Time Scale로 변환하는 코드
...
function createResizeDragElement(note, leftPosition, noteId, type) {
  //Melody, Beat 노트 생성
...
function newClipCreater() {
  if (current_clip_type == MusicClipType.Melody) {
    ...
    template_clip = new TemplateClip(
      Template_clip_array.length,
      duration_track
    );
    templateConnectToVisualAndSound(template_clip);
...
/* Clip Event Code */
document
  .getElementById("trackClipCreateButton")
  .addEventListener("click", function () {
    newClipCreater();
  });
document
  .getElementById("sheetMusicTemplateButton")
  .addEventListener("click", function () {
    if (template_clip.get_Clip_id() != Template_clip_array.length) {
      console.log(
        "New Template Clip Create",
        template_clip.get_Clip_id(),
        Template_clip_array.length
      );
      template_clip = new TemplateClip(
        Template_clip_array.length,
...
document
  .getElementById("sheetMusicSaveButton")
  .addEventListener("click", function () {
    if (current_clip_type == MusicClipType.Melody) {
      // Melody Save
...
document
  .getElementById("sheetMusicPlayButton")
  .addEventListener("click", function () {
    startRecording();
  });
document
  .getElementById("sheetMusicPauseButton")
  .addEventListener("click", function () {
    stopRecording();
    stopAllNotePlayer();
    stopAllNotePlayer();
...
document
  .getElementById("sheetMusicDeleteButton")
  .addEventListener("click", function () {
    if (noteClickIndex != -1) {
      if (current_clip_type == MusicClipType.Melody) {
...
document
  .getElementById("sheetMusicAllDeleteButton")
  .addEventListener("click", function () {
    if (window.confirm("정말 클립 내용을 모두 삭제하시겠습니까?")) {
      if (current_clip_type == MusicClipType.Melody) {
        melody_clip = new MusicClip(
          MusicClipType.Melody,
          melody_clip.getClipId(),
          duration
        );
        clearNoteClip(MusicClipType.Melody);
        loadClip(melody_clip, melody_clip.getDuration());
...
SyntheysizerEvents.addEventListener("pianoKeyInput", function (e) {
  ...
  if (doubleChecker % 2 == 0) {
    if (play_state && current_clip_type == MusicClipType.Melody) {
      ...
      let NoteItem = createResizeDragElement(
        e.detail.note,
        time_to_px(currentTime, currentClipDuration()),
        melody_clip.getNoteIndex(),
        MusicClipType.Melody
      );
...
function loadClip(MusicClip, duration) {
  ...
  if (MusicClip.getClipType() == MusicClipType.Melody) {
    ...
    for (let i = 0; i < NoteSet.length; i++) {
      let NoteItem = createResizeDragElement(
        NoteSet[i],
        time_to_px(TimeSet[i][0], duration),
        i,
        MusicClipType.Melody
      );
...
function changeMusicClip(noteIndex, deltaTimeset) {
  // 노트 위치, 크기 편집을 클립 시간에 반영
  if (current_clip_type == MusicClipType.Melody) {
...
function changelyricsClip(noteIndex, deltaTimeset) {
  // 노트 위치, 크기 편집을 클립 시간에 반영
  if (
    current_clip_type == MusicClipType.Melody ||
    current_clip_type == MusicClipType.Lyrics
...
function createClipBox(musicClip) {
  //Melody, Beat 노트 생성
  ...
  dragdrop.addEventListener("click", function () {
    console.log(
      "current Clip Type:",
      dragdrop.getAttribute("clip_type"),
      "current Clip id",
      dragdrop.getAttribute("clip_id")
    );
    loadFromTrackToMusicClip(
      dragdrop.getAttribute("clip_type"),
      dragdrop.getAttribute("clip_id")
    );
  });
...
function createTemplateClipBox(template_clip) {
  ...
  dragdrop_template.addEventListener("click", function () {
    //console.log("current Clip Type:", dragdrop_template.getAttribute("clip_type"),"current Clip id", dragdrop_template.getAttrinclibute("clip_id"))
...
function createTrackClipObject(
  dropzoneName,
  clipType,
  clip_id,
  duration,
  ...
  trackClip.style.width =
    time_to_px(duration, duration_track, track_box_width, 0) + "px";
  ...
  trackClip.addEventListener("click", function () {
    console.log(
      "type:",
      trackClip.getAttribute("box_type"),
      "id",
      trackClip.getAttribute("box_id")
    );
    trackClickIndex = trackClip.getAttribute("box_id");
    trackClickType = trackClip.getAttribute("box_type");
  });
  boxItem.appendChild(trackClip);
  trackClip.style.left =
    time_to_px_Scale(startTime, duration_track, track_box_width, 0) + "px";
...
function createTrackClipObject_template(dropzoneName, clip_id, box_id) {
  ...
  trackClip.style.width =
    time_to_px(60, duration_track, track_box_width, 0) + "px";
  let boxItem = document.getElementById(dropzoneName);
  ...
  boxItem.appendChild(trackClip);
  return px_to_time_Scale(
    trackClip.offsetLeft,
    duration_track,
    track_box_width,
    0
  );
...
$("#slider_track").slider({
  ...
  slide: function (event, ui) {
    ...
    document.getElementById("trackTimeShower").innerHTML =
      getTimeString(currentTrackTime);
...
function updateTime2() {
  ...
  if (bpmPlayNumber > 4) {
    ...
    $("#slider_track").slider(
      "value",
      time_to_px(currentTrackTime, duration_track, track_box_width, 0)
    );
    document.getElementById("trackTimeShower").innerHTML =
      getTimeString(currentTrackTime);
    timeLine3.style.left =
      time_to_px(currentTrackTime, duration_track, track_box_width, 0) + "px";
    ...
    if (cur_track_set[1].length > 0) {
      musicPlayerMelodyClip(
        currentTrackTime - cur_track_set[1][0],
        Melody_clip_array[cur_track_set[1][1]]
      );
      if (
        current_clip_type == MusicClipType.Melody ||
    ...
    if (cur_track_set[2].length > 0) {
      musicPlayerBeatClip(
        currentTrackTime - cur_track_set[2][0],
        Beat_clip_array[cur_track_set[2][1]]
      );
...
function musicPlayerMelodyClip(currentTime, melody_clip) {
  //음이나 비트 소리를 재생하는 코드
  let currentNote = melody_clip.getcurrentNoteSet(currentTime);
...
function musicPlayerBeatClip(currentTime, beat_clip) {
  //음이나 비트 소리를 재생하는 코드
  let currentBeat = beat_clip.getcurrentNoteSet(currentTime);
  for (let beat of currentBeat) {
    beat_player(beat);
...
function templateConnectToVisualAndSound(inputClip) {
  const event = new CustomEvent("templateLoad", {
    detail: inputClip.get_dial(),
  });
  SyntheysizerEvents.dispatchEvent(event);
}
function clearAllBoxClip() {
  // 편집기에 모든 노트 제거
  InitializeAllTrack();
...
document
  .getElementById("trackMusicPlayButton")
  .addEventListener("click", function () {
    startTrack();
  });
document
  .getElementById("trackMusicPauseButton")
  .addEventListener("click", function () {
    stopTrack();
    stopAllNotePlayer2();
  });
document
  .getElementById("trackMusicDeleteButton")
  .addEventListener("click", function () {
    TrackObject.deleteClip(trackClickType, trackClickIndex);
    InitializeAllTrack();
    loadTrack(TrackObject);
  });
function loadTrack(MusicTrack) {
  // Track을 편집기에 반영
  ...
  for (let i = 0; i < MelodyClipId.length; i++) {
    createTrackClipObject(
      "melody-dropzone",
      MusicClipType.Melody,
      MelodyClipId[i],
      Melody_clip_array[MelodyClipId[i]].getDuration(),
      i,
      MelodyTimeset[i][0]
    );
  }
  for (let i = 0; i < BeatClipId.length; i++) {
    createTrackClipObject(
      ...
      MusicClipType.Beat,
      BeatClipId[i],
      Beat_clip_array[BeatClipId[i]].getDuration(),
      i,
      BeatTimeset[i][0]
    );
  }
...
function loadFromTrackToMusicClip(clip_type, clip_id) {
  if (clip_type == MusicClipType.Melody) {
    if (current_clip_type == MusicClipType.Lyrics) {
      //클릭 당시 Lyrics 씬일 때만 적용
      lyricsTypeSceneChanger();
      clearNoteClip(MusicClipType.Melody);
      melody_clip = Melody_clip_array[clip_id];
      loadClip(melody_clip, melody_clip.getDuration());
    } else {
      // 나머지는 다 Melody 씬으로 이동
      melodyTypeSceneChanger();
      clearNoteClip(MusicClipType.Melody);
      melody_clip = Melody_clip_array[clip_id];
      loadClip(melody_clip, melody_clip.getDuration());
...
function loadLyrics(jsonObject) {
  ...
  for (let i = 0; i < trackIdSet.length; i++) {
    ...
    for (let j = 0; j < LyricsSet[clipIndex].length; j++) {
      ...
      AllLyricsTimeSet.push([
        LyricsTimeset[clipIndex][j][0] + addTime,
        LyricsTimeset[clipIndex][j][1] + addTime,
      ]);
...
/*------------------------------------------laod MIDI Code -------------------------------------------- */
document
  .getElementById("sheetMusicLoadButton")
  .addEventListener("click", function () {
    FileInput.click();
  });
document
  .getElementById("MoodLoadButtonMood")
  .addEventListener("click", function () {
    FileInput.click();
  });
...
//     });
// }
function getMidiResolution(data) {
  // MIDI 파일 헤더는 처음 14바이트에 위치합니다.
  const headerChunk = data.slice(0, 14);
  console.log("Header chunk:", headerChunk);

  // 헤더의 9번째 바이트부터 2바이트를 해상도 값으로 가져옵니다.
  const resolutionBytes = headerChunk.slice(12, 14);
  console.log("Resolution bytes:", resolutionBytes);

  // 해상도 값은 두 바이트를 Little-endian 형식으로 저장되어 있습니다.
  const resolution = (resolutionBytes[0] << 8) + resolutionBytes[1];
  return resolution;
}
...
function fetchMidiContent() {
  ...
  fetch("/get_midi_data")
    .then((response) => {
      ...
      return response.arrayBuffer(); // 바로 ArrayBuffer로 받기
    })
    .then((data) => {
      // MIDI 데이터를 사용하여 원하는 동작 수행
      console.log("Handling MIDI Content:", data);
...
function setMidiToBeatClip(inputLog) {
  ...
  if (input) {
    console.log(
      ...
      note.slice(0, -1),
      "index :",
      midiBeatNote.indexOf(note.slice(0, -1)),
      "time:",
      time / beatScaling
    );
    if (midiBeatNote.indexOf(note.slice(0, -1)) != -1) {
      beat_clip.setBeatInput(
        midiBeatNote.indexOf(note.slice(0, -1)),
        time / beatScaling
      );
    } else {
...
function getMidiLine(midiLine) {
  if (midiLine[0] >= 144 && midiLine[0] <= 154) {
    return [
      midiLine[0] - 144,
      true,
      pitch2NoteMidi(midiLine[1]),
      midiLine.tt / midiBPM / 4,
    ];
...
function pitch2NoteMidi(input_pitch) {
  ...
  return (
    noteType[(input_pitch - 12) % 12] +
    String(parseInt((input_pitch - 12) / 12))
  );
}
...
document
  .getElementById("LyricsPushButton")
  .addEventListener("click", function () {
    ...
    if (lyricsText != "") {
      ...
      melody_clip.setLyrics(lyricsText, [
        lyricsLastTime,
        lyricsLastTime + lyricsDefaultTime,
      ]);
      createLyricsObject(
        lyticsId,
        lyricsText,
...
function createLyricsObject(
  ...
  if (lyricsText != "") {
    ...
    lyricsNote.style.width =
      time_to_px_Scale(endTime - startTime, currentClipDuration()) + "px";
    ...
    lyricsNote.addEventListener("click", function () {
      ...
      let lyricsVideoId = melody_clip.getLyricsVideoId(
        lyricsNote.getAttribute("note_id")
      );
      if (lyricsVideoId != -1) {
    ...
    lyricsNote.addEventListener("click", function history() {
      ...
      // clickedHistory[-2] != clickedHistory[-1]
      if (clickedHistory.length == 1) {
        //하나 이상 클릭했을 때
        // console.log('히스토리', clickedHistory);
        nowClicked = document.getElementById(clickedHistory[0]);
        nowClicked.style.borderWidth = "thick";
      } else if (
        clickedHistory.length > 1 &&
        clickedHistory[clickedHistory.length - 1] !=
          clickedHistory[clickedHistory.length - 2]
      ) {
        nowClicked = document.getElementById(
          clickedHistory[clickedHistory.length - 1]
        );
        previousClicked = document.getElementById(
          clickedHistory[clickedHistory.length - 2]
        );
...
function showLyrics(currentTime) {
  let [currentLyricsText, currentLyricsIndex] =
    melody_clip.getLyrics(currentTime);
  ...
  .resizable({
    ...
    listeners: {
      move: function (event) {
        ...
        changelyricsClip(event.target.getAttribute("note_id"), [
          px_to_time_Scale(event.deltaRect.left, currentClipDuration()),
          px_to_time_Scale(
            event.deltaRect.left + event.deltaRect.width,
            currentClipDuration()
          ),
        ]);
...
function dragMoveListener_lyrics(event) {
  ...
  changelyricsClip(event.target.getAttribute("note_id"), [
    px_to_time_Scale(event.dx, currentClipDuration()),
    px_to_time_Scale(event.dx, currentClipDuration()),
  ]);
...
document.getElementById("bpm").addEventListener("change", function () {
  saveStepTime =
    (60 / document.getElementById("bpm").value) *
    parseInt(document.getElementById("beatSelect").value);
  removeAllVerticalGrid();
...
function createVerticalGrid(clipType, stepSize = -1) {
  ...
  if (stepSize == -1) {
    ...
    if (clipType == MusicClipType.Melody) {
      stepSize = time_to_px_Scale(
        saveStepTime,
        melody_clip.getDuration(),
        clip_box_width - clip_start_px,
        0
      );
    } else {
      stepSize = time_to_px_Scale(
        saveStepTime,
        beat_clip.getDuration(),
        clip_box_width - clip_start_px,
        0
      );
    }
  ...
  .resizable({
    ...
    listeners: {
      move: function (event) {
        ...
        changeMusicClip(event.target.getAttribute("note_id"), [
          px_to_time_Scale(event.deltaRect.left, currentClipDuration()),
          px_to_time_Scale(
            event.deltaRect.left + event.deltaRect.width,
            currentClipDuration()
...
// Track Template
interact(".resize-drag_clip").resizable({
...
function dragMoveListener_clip(event) {
  ...
  TrackObject.editMusicClip(
    event.target.getAttribute("box_type"),
    event.target.getAttribute("box_id"),
    px_to_time(event.dx, duration_track, track_box_width, 0)
  );
  // console.log("Add Plus :", px_to_time(x, duration), "id:", target.getAttribute("note_id"));
  // update the posiion attributes
  target.setAttribute("data-x", x);
...
interact(".template-dropzone").dropzone({
  ...
  ondragenter: function (event) {
    ...
    if (trackActivaqte) {
      ...
      const clipTime = createTrackClipObject_template(
        "template-dropzone",
        tempalteId,
        TrackObject.getTempalateId()
      );
...
interact(".melody-dropzone").dropzone({
  ...
  ondragenter: function (event) {
    ...
    if (trackActivaqte) {
      ...
      event.target.classList.remove("drop-active");
      event.target.classList.remove("drop-target");
      let Tmp_clip = get_clip(
        event.relatedTarget.getAttribute("clip_type"),
        event.relatedTarget.getAttribute("clip_id")
      );
      let [lastMelodyId, lastMelodyTime] = TrackObject.getLastClipData(
        MusicClipType.Melody
      );
      let trackLastTime = 0;
      if (lastMelodyId != null) {
        trackLastTime =
          lastMelodyTime[0] + Melody_clip_array[lastMelodyId].getDuration();
      }
      console.log("trackLastTime", trackLastTime, Tmp_clip.getDuration());
      if (trackLastTime + Tmp_clip.getDuration() < duration_track) {
        createTrackClipObject(
          "melody-dropzone",
          Tmp_clip.getClipType(),
          Tmp_clip.getClipId(),
          Tmp_clip.getDuration(),
          TrackObject.getMelodyId(),
          trackLastTime
        );
...
interact(".beat-dropzone").dropzone({
  ...
  ondragenter: function (event) {
    ...
    if (trackActivaqte) {
      ...
      let [lastBeatId, lastBeatTime] = TrackObject.getLastClipData(
        MusicClipType.Beat
      );
      let trackLastTime = 0;
      if (lastBeatId != null) {
        trackLastTime =
          lastBeatTime[0] + Beat_clip_array[lastBeatId].getDuration();
      }
      if (trackLastTime + Tmp_clip.getDuration() < duration_track) {
        createTrackClipObject(
          "beat-dropzone",
...
// LRC파일
function generateLrcFile(lyrics, audioLength, TimeSet) {
  let lrcContent = "";
  let lrcLine = "";
  let before = 0;
  for (let i = 0; i < lyrics.length; i++) {
    ...
    if (line !== "") {
      const startTime = TimeSet[i][0];
      const endTime = TimeSet[i][1];
      if (i !== 0 && before !== startTime) {
        let lrcLine = `[${formatTime(before)}]${"간주중"}\n`;
        lrcContent += lrcLine;
      }
      lrcLine = `[${formatTime(startTime)}]${line}\n`;
      lrcContent += lrcLine;
      before = endTime;
    }
  }

  if (before !== audioLength) {
    let lrcLine = `[${formatTime(before)}]${"후주중"}\n`;
    lrcContent += lrcLine;
  }
...
function formatTime(seconds) {
  ...
  const remainingMilliseconds = Math.round((seconds % 1) * 100);

  return `${padZero(minutes)}:${padZero(remainingSeconds)}.${padZero(
...
document.getElementById("LRCSaveButton").addEventListener("click", async () => {
  console.log(melody_clip);
  console.log(melody_clip.getDuration());
  const lyrics = melody_clip.lyricSet;
  const audioLength = melody_clip.getDuration(); // 오디오 길이 (밀리초)
  const TimeSet = melody_clip.lyrictimeSet;
  const lrcContent = generateLrcFile(lyrics, audioLength, TimeSet);
